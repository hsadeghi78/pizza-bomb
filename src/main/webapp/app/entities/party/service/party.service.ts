...
export class PartyService {
  public resourceUrl = this.applicationConfigService.getEndpointFor('api/parties');
  ...
  protected convertDateFromClient(party: IParty): IParty {
    return Object.assign({}, party, {
      activationDate: party.activationDate?.isValid() ? party.activationDate.format(DATE_FORMAT) : undefined,
      expirationDate: party.expirationDate?.isValid() ? party.expirationDate.format(DATE_FORMAT) : undefined,
    });
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((party: IParty) => {
        party.activationDate = party.activationDate ? dayjs(party.activationDate) : undefined;
        party.expirationDate = party.expirationDate ? dayjs(party.expirationDate) : undefined;
      });
    }
    return res;
  }
